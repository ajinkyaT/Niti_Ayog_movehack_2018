...
def createInstanceImage(annotation, encoding):
    ...
    # the background
    if encoding == "ids":
        backgroundId = name2label['obs-str-bar-fallback'].id
    elif encoding == "trainIds":
        backgroundId = name2label['obs-str-bar-fallback'].trainId
    else:
...
def json2instanceImg(inJson,outImg,encoding="trainIds"):

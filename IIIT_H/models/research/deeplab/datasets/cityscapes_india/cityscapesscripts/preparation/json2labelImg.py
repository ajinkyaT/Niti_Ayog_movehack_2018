...
def createLabelImage(annotation, encoding, outline=None):
    ...
    # the background
    if encoding == "ids":
        background = name2label['obs-str-bar-fallback'].id
    elif encoding == "trainIds":
        background = name2label['obs-str-bar-fallback'].trainId
    elif encoding == "color":
        background = name2label['obs-str-bar-fallback'].color
    else:
    ...
    for obj in annotation.objects:
        ...
        try:
            if outline:
                if name2label[label].hasInstances:
...
def json2labelImg(inJson,outImg,encoding="trainIds"):
    ...
    #labelImg.save( outImg.split('.')[0]+'_diff_cls.png' )

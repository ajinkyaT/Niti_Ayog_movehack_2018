...
def generate_batch(temp_id,batch_size, horizontal_flip=False, vertical_flip=False):
    ...
    y = np.load('../data/data_files/'+temp_id+'_mask.npy') 
...
if __name__ == '__main__':
    ...
    if train_pretrained_model:
        model = read_model()
        model.compile(optimizer='Adadelta', loss='binary_crossentropy', metrics=['binary_crossentropy', jaccard_coef_int,'accuracy'])
    else:
        #Building model
        model = get_unet0()
        model.compile(optimizer='Adadelta', loss='binary_crossentropy', metrics=['binary_crossentropy', jaccard_coef_int,'accuracy'])

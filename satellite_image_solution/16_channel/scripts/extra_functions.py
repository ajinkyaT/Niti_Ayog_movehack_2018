...
#%%
def one_layer_mask(mask):
    num_class, height, width = mask.shape[:]
    ...
    return mask_
#%%
def generate_mask(image_id, height, width, num_mask_channels, train=train_wkt):
    """
    :param image_id:
    :param height:
    :param width:
    ...
    # mask has a layer for each class
#    if num_mask_channels ==1:
#        mask_ = one_layer_mask(mask)
#    else:
    mask_ = mask
    return mask_
...
def read_image_16(image_id):
    ...
    #result = np.transpose(np.concatenate([rescaled_P, ndwi, savi, evi, ccci, img_3], axis=2), (2, 0, 1))
